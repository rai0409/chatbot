...
rag_core/store.py uses single-node local Chroma. pgvector/Qdrant are candidate
production backends. This is an ANALYSIS/decision prompt; do not migrate unless
explicitly scoped and tested.

## Goal

Analyze Chroma vs pgvector vs Qdrant for the production backend based on the
existing store boundary, the Chroma where-clause behavior (Prompt035), and
current tests. Recommend stay-on-Chroma for the first annual contract vs a
migration spike.

## Scope / deliverables

- A decision report: current store coupling, adapter-boundary readiness,
  on-prem/offline fit, ops/HA implications, migration risk, and a recommendation
  with done-criteria for any future spike.
- No migration; no new dependency; analysis only.
- docs/reports/prompt067_vectorstore_production_backend_decision_pgvector_qdrant.md.

## Tests / checks

    python -m pytest tests/test_chroma_where_builder.py tests/test_durable_multitenant_persistence.py -q
...
PASS -> commit message "prompt067 vectorstore production backend decision pgvector qdrant", tag "prompt067-vectorstore-production-backend-decision-pgvector-qdrant".
