# Prompt065: Design-Partner PoC Results to Annual Contract Report
...
After a design-partner PoC (Prompts056-060 enable it), a structured report is
needed to decide on an annual contract. This prompt creates the report PACKAGE
TEMPLATE (real results filled in later; none fabricated now).

## Goal

Create the report package that turns a design-partner PoC into an annual-contract
decision: measured-results template, limitations, user feedback, incidents,
unresolved risks, pricing/renewal proposal, expansion plan, and a go/no-go
checklist.

## Scope / deliverables

- docs/sales/ + docs/reports/ templates with placeholders; explicit "fill with
  real measured data; do not fabricate".
...
PASS -> commit message "prompt065 design partner poc results to annual contract report", tag "prompt065-design-partner-poc-results-to-annual-contract-report".
