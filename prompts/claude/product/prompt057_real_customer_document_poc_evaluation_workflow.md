...
Prompt039 measured abstain/answer behavior on SYNTHETIC manufacturing data. The
P0 commercial unknown is accuracy on the customer's real (sanitized) documents,
which can only be measured during a PoC. No real customer data may be committed.

## Goal

Create a repeatable real-document PoC evaluation WORKFLOW (templates + tooling)
that an operator runs on sanitized/customer-approved documents to measure
answer / abstain / error / citation quality with human review - committing only
templates and tooling, never customer data.

## Scope

- Question-set templates (answerable / abstain / out-of-corpus / citation-
  required / approved-QA) mirroring eval/cases schema, with placeholders.
- A measurement procedure reusing eval.runner + a human-review scoring sheet
  (answer correct? cited? abstained appropriately?), output to a gitignored
  runs/ location.
- A PoC result REPORT TEMPLATE (metrics table + limitations + reviewer notes).
- Strict data handling: customer documents and outputs stay in gitignored/local
  paths; .gitignore guidance; nothing customer-specific committed.

## Required deliverables

- Templates + a thin safe helper/check; tests on the TEMPLATES with synthetic
  data only; docs/reports/prompt057_real_customer_document_poc_evaluation_workflow.md.

## Tests / checks

    python -m pytest tests/test_poc_evaluation_workflow.py -q
...
PASS -> commit message "prompt057 real customer document poc evaluation workflow", tag "prompt057-real-customer-document-poc-evaluation-workflow".
